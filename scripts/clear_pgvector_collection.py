"""Delete chunks from one app-level collection in the pgvector store.

pgvector stores every app-level collection (papers, docs, ...) as rows in ONE
shared table, distinguished by a `collection` column — there is no
per-app-collection DROP. This deletes document-by-document via the
VectorStore port's delete(document_id), since document_id already encodes
which app collection it belongs to (e.g. "papers/50455611c3ed8b48").
...
Usage:
    uv run python scripts/clear_pgvector_collection.py --collection papers
    uv run python scripts/clear_pgvector_collection.py --collection papers --yes
    uv run python scripts/clear_pgvector_collection.py --collection papers \\
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Delete chunks from one collection in the pgvector store "

...
class DoclingNormalizer:
    async def normalize(
        ...
        sections = tuple(
            DocumentSection(
                title=s.get("title", ""), level=int(s.get("level", 0)),
                kind=s.get("kind", "prose"), text=s.get("text", ""), order=i,
            )
        ...
        return [
            NormalizedDocument(
                ...
                title=payload.get("title", ""), format=parsed.format, parser=parsed.parser,
                content_hash=hash_bytes(parsed.text.encode()),
                sections=sections, ingested_at=datetime.now(timezone.utc).isoformat(),
            )
        ]
